...
const testDir = '../../../'
const inputDirname = fileURLToPath(new URL(`${testDir}/simple-grant/unwrapped`, import.meta.url))
const outputDir = fileURLToPath(new URL(`${testDir}/simple-grant/wrapped`, import.meta.url))
if (!fs.existsSync(outputDir)){
  fs.mkdirSync(outputDir);
}
const agreement = JSON.parse(readFileSync(join(inputDirname, 'simple-grant.json'), 'utf-8'));
  ...
  try {
    ...
    const agreementParams = {
      credential: {
        ...
        credentialSubject: {
          id: "did:example:simple-grant-recipient-1",
