...
Handlers.add(
  "VerifyTx",
  Handlers.utils.hasMatchingTag("Action", "VerifyTx"),
  function (msg)
    local data = json.decode(msg.Data)
    -- Extract the proof data
    local txHash = data.TxHash
    local transactionIndex = data.TxIndex
    local transactionRoot = data.TxRoot
    local proof = data.Proof
    local value = data.Value
    
    local isValid = verifyProof(txHash, transactionIndex, transactionRoot, proof, value)
    print('The provided proof for ' .. txHash .. ' is ' .. (isValid and "valid" or "not valid"))
    msg.reply({ Data = msg.From })
  end
)