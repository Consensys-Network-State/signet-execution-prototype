...
async function getTransactionProof(txHash) {
    ...
    console.log(`Found transaction at index ${receipt.transactionIndex} in block ${block.number}`);
    ...
    for (let i = 0; i < block.transactions.length; i++) {
        ...
        try {
            ...
            // Log for debugging
            if (i === parseInt(receipt.transactionIndex)) {
    ...
    // 5. Generate Proof
    const txIndex = RLP.encode(parseInt(receipt.transactionIndex));
